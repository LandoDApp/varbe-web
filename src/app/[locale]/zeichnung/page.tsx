...
export default function ZeichnungPage() {
    const t = useTranslations('categories.zeichnung');
    ...
    useEffect(() => {
        const fetchArtworks = async () => {
            try {
                ...
                const drawingKeywords = ['zeichnung', 'drawing', 'skizze', 'sketch', 'grafik', 'graphic', 'bleistift', 'pencil', 'tusche', 'ink'];
                const filtered = allArtworks.filter(a => {
                    if (a.adminApprovalStatus !== 'approved') return false;
                    if (a.status === 'ended') return false;
                    if (a.status === 'sold' && (a.quantity === undefined || a.quantity === 0)) return false;
                    
                    const titleLower = a.title.toLowerCase();
                    const descLower = a.description.toLowerCase();
                    return a.category === 'other' || 
                           drawingKeywords.some(keyword => titleLower.includes(keyword) || descLower.includes(keyword));
                });
            ...
            <div className="container mx-auto px-4 py-12 max-w-7xl">
                    ...
                    <>
                        ...
                        {artworks.length > 12 && (
                            <div className="text-center mt-8">
                                <Link href="/marketplace?category=other">
                ...
                <section className="mt-12 space-y-6 font-body text-lg">
                    ...
                    <ul className="list-disc list-inside ml-4 space-y-2">
                        <li><strong>{t('seoTechniques.pencil').split(':')[0]}:</strong> {t('seoTechniques.pencil').split(':')[1]}</li>
                        <li><strong>{t('seoTechniques.ink').split(':')[0]}:</strong> {t('seoTechniques.ink').split(':')[1]}</li>
                        <li><strong>{t('seoTechniques.charcoal').split(':')[0]}:</strong> {t('seoTechniques.charcoal').split(':')[1]}</li>
                        <li><strong>{t('seoTechniques.pastel').split(':')[0]}:</strong> {t('seoTechniques.pastel').split(':')[1]}</li>
                        <li><strong>{t('seoTechniques.mixed').split(':')[0]}:</strong> {t('seoTechniques.mixed').split(':')[1]}</li>
                    </ul>
                    <h3 className="text-2xl font-heading mt-8">{t('seoStorageTitle')}</h3>
                    <p>
                        {t('seoStorageText')}
                    </p>
                </section>
