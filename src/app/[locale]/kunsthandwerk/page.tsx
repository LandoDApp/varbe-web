...
export default function KunsthandwerkPage() {
    const t = useTranslations('categories.kunsthandwerk');
        ...
        <div className="min-h-screen bg-white">
            ...
            <div className="container mx-auto px-4 py-12 max-w-7xl">
                    ...
                    <>
                        ...
                        {artworks.length > 12 && (
                            <div className="text-center mt-8">
                                <Link href="/marketplace?category=crafts">
                ...
                <section className="mt-12 space-y-6 font-body text-lg">
                    ...
                    <h3 className="text-2xl font-heading mt-8">{t('seoCategoriesTitle')}</h3>
                    <p>
                        {t('seoCategoriesText')}
                    </p>
                    <ul className="list-disc list-inside ml-4 space-y-2">
                        <li><strong>{t('seoCategories.jewelry').split(':')[0]}:</strong> {t('seoCategories.jewelry').split(':')[1]}</li>
                        <li><strong>{t('seoCategories.textiles').split(':')[0]}:</strong> {t('seoCategories.textiles').split(':')[1]}</li>
                        <li><strong>{t('seoCategories.woodwork').split(':')[0]}:</strong> {t('seoCategories.woodwork').split(':')[1]}</li>
                        <li><strong>{t('seoCategories.ceramics').split(':')[0]}:</strong> {t('seoCategories.ceramics').split(':')[1]}</li>
                        <li><strong>{t('seoCategories.metalwork').split(':')[0]}:</strong> {t('seoCategories.metalwork').split(':')[1]}</li>
                        <li><strong>{t('seoCategories.glasswork').split(':')[0]}:</strong> {t('seoCategories.glasswork').split(':')[1]}</li>
                        <li><strong>{t('seoCategories.leather').split(':')[0]}:</strong> {t('seoCategories.leather').split(':')[1]}</li>
                        <li><strong>{t('seoCategories.mixed').split(':')[0]}:</strong> {t('seoCategories.mixed').split(':')[1]}</li>
                    </ul>
                    <h3 className="text-2xl font-heading mt-8">{t('seoCareTitle')}</h3>
                    <p>
                        {t('seoCareText')}
                    </p>
                </section>
