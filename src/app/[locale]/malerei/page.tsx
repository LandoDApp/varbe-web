...
export default function MalereiPage() {
    const t = useTranslations('categories.malerei');
    ...
    const acrylicCount = artworks.filter(a => a.technique === 'acrylic').length;
    const oilCount = artworks.filter(a => a.technique === 'oil').length;
    const watercolorCount = artworks.filter(a => a.technique === 'watercolor').length;
        ...
        <div className="min-h-screen bg-white">
            ...
            <div className="container mx-auto px-4 py-12 max-w-7xl">
                ...
                <section className="mb-12">
                    <h2 className="text-3xl font-heading mb-6">{t('subcategoriesTitle')}</h2>
                    <div className="flex flex-wrap gap-4">
                        {acrylicCount > 0 && (
                            <Link href="/marketplace?category=painting&technique=acrylic">
                                <Button variant="accent" className="text-lg px-6 py-3">
                                    Acrylmalerei ({acrylicCount} Werke)
                                </Button>
                            </Link>
                        )}
                        {oilCount > 0 && (
                            <Link href="/marketplace?category=painting&technique=oil">
                                <Button variant="primary" className="text-lg px-6 py-3">
                                    Ölmalerei ({oilCount} Werke)
                                </Button>
                            </Link>
                        )}
                        {watercolorCount > 0 && (
                            <Link href="/marketplace?category=painting&technique=watercolor">
                                <Button variant="accent" className="text-lg px-6 py-3">
                                    Aquarell ({watercolorCount} Werke)
                                </Button>
                            </Link>
                        )}
                    </div>
                ...
                {loading ? (
                    ...
                    <div className="card-comic bg-white p-12 text-center">
                        ...
                        <Link href="/marketplace/create">
                            <Button variant="accent">{tCommon('createListing')}</Button>
                        </Link>
                    </div>
                ) : (
                    <>
                        ...
                        {artworks.length > 12 && (
                            <div className="text-center mt-8">
                                <Link href="/marketplace?category=painting">
                ...
                <section className="mt-12 space-y-6 font-body text-lg">
                    ...
                    <h3 className="text-2xl font-heading mt-8">{t('seoHangingTitle')}</h3>
                    <p>
                        {t('seoHangingText')}
                    </p>
                </section>
